...
const Header = () => {
    ...
    // Scroll event handler for header show/hide
    useEffect(() => {
        const controlHeader = () => {
    ...
    const projectCategories = [
        {
            ...
            subItems: [
                { title: "Residential", path: "/malbar-prime-residential" },
                { title: "Commercial", path: "/Malbar-Prime" },
    ...
    const workCategories = [
        ...
        { title: "Petroleum", path: "/Petrolpump" },
        { title: "Bricks", path: "/BricksShowcase" }
    ]

    const toggleSubCategory = (title) => {
        // Check if the category has the disableDropdown flag
        const category = projectCategories.find(cat => cat.title === title);
        if (category && category.disableDropdown) {
            return; // Don't toggle if dropdown is disabled
        }
    ...
    const toggleMobileProjectCategory = (title) => {
        // Check if the category has the disableDropdown flag
        const category = projectCategories.find(cat => cat.title === title);
        if (category && category.disableDropdown) {
            return; // Don't toggle if dropdown is disabled
        }
        setMobileProjectCategory(mobileProjectCategory === title ? null : title)
    }
    ...
    return (
        ...
        >
            <div className="w-full flex items-center justify-between px-4 md:px-8 py-4 relative  mx-auto bg-white shadow-md">
                {/* Logo */}
                                ...
                                >
                                    {projectCategories.map((category) => (
                                        <div key={category.title} className="border-b border-gray-100 last:border-b-0">
                                            {/* Category header - Direct link if disableDropdown is true */}
                                            <motion.div
                                                variants={itemVariants}
                                                className="group"
                                            >
                                                {category.disableDropdown ? (
                                                    <a
                                                        href={category.path}
                                                        className={`px-5 py-4 ${isCategoryActive(category) ? 'text-orange-500' : 'text-gray-700 hover:text-orange-500'} transition-colors duration-200 flex justify-between items-center`}
                                                    >
                                            ...
                                            {/* Sub-items - Only render if not disableDropdown */}
                                            {!category.disableDropdown && (
                                                <AnimatePresence>
                                                ...
                                                {projectCategories.map((category) => (
                                                    <div key={category.title} className="border-l-2 border-gray-200 pl-4">
                                                        {category.disableDropdown ? (
                                                            <a
                                                                href={category.path}
                                                                className={`block ${isCategoryActive(category) ? 'text-orange-500' : 'text-gray-600 hover:text-orange-500'} transition-colors duration-200 py-3`}
                                                            >
                                                                {category.title}
                                                            </a>
                                                        ) : (
                                                            <>
                                                                <div
                                                                    ...
                                                                    onClick={(e) => {
                                                                        ...
                                                                        toggleMobileProjectCategory(category.title);
                                                                    }}
                                                                >
